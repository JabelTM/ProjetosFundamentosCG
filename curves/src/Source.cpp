...
int main()
{
  ...
  GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Curves", nullptr, nullptr);
  ...
  float speed = 0.03f; // Velocidade de movimento
  float time = 0.0f;
  ...
  while (!glfwWindowShouldClose(window))
{
  ...
  time += speed; // Incrementa o tempo de acordo com a velocidade

  int index = static_cast<int>(time) % nbCurvePoints;
  glm::vec3 pointOnCurve = bezier.getPointOnCurve(index);
