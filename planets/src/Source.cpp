...
const string MERCURY_OBJ_FILE_PATH = ASSETS_FOLDER + "planetMercury.obj";
const string EARTH_OBJ_FILE_PATH = ASSETS_FOLDER + "planetEarth.obj";
...
int main()
{
  ...
  GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Planets", nullptr, nullptr);
  ...
  ParsedObj parsedMercuryObj = parseOBJFile(MERCURY_OBJ_FILE_PATH);
  vector<Material> mercuryMaterials = readMTLFile(ASSETS_FOLDER, parsedMercuryObj.mtlFileName);
  Material mercuryMaterial = mercuryMaterials[0];
  GLuint mercuryTextureId = loadTexture(mercuryMaterial.texturePath);
  Geometry mercuryGeometry = setupGeometry(parsedMercuryObj.vertices);
  GLuint MERCURY_VAO = mercuryGeometry.VAO;
  int mercuryVerticesCount = mercuryGeometry.verticesCount;

  Mesh mercury;
  mercury.initialize(MERCURY_VAO, mercuryVerticesCount, &shader, mercuryTextureId, glm::vec3(0.0f,0.0f,0.0f), glm::vec3(0.1f,0.1f,0.1f));
  mercury.setShouldRotateY(true);
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    camera.update();
		...
		mercury.updatePosition(pointOnCurve);
    
		mercury.update();
		mercury.draw(mercuryMaterial);
  ...
  glDeleteVertexArrays(1, &MERCURY_VAO);
